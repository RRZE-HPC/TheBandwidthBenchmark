...
plot '${dat_file}' using 1:3 with linespoints title "${kernel}" pointsize 2 pointtype 7 lw 6
EOF

        # Append to combined plot
        combined_plot_commands+=("'${dat_file}' using 1:3 with linespoints title '${kernel}' pointtype 7 lw 6,")
    else
